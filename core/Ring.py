...
class Ring(Object2D):
    ...
    def draw_ring(self, number_of_points):
        ...
        # else:
        #     for i in range(number_of_points + 1):
        #         points.append(pya.Point.new(
        #             x + radius * np.sin(angle * i / number_of_points + theta_start) + thickness * np.sin(
        #                 (theta_start + theta_stop) / 2) / 2,
        #             y + (radius + elliptical / 2) * np.cos(
        #                 angle * i / number_of_points + theta_start) + thickness * np.cos(
        #                 (theta_start + theta_stop) / 2) / 2))
        #     for i in range(number_of_points + 1):
        #         points.append(pya.Point.new(
        #             x + radius * np.sin(-angle * i / number_of_points + theta_stop) - thickness * np.sin(
        #                 (theta_start + theta_stop) / 2) / 2,
        #             y + (radius + elliptical / 2) * np.cos(
        #                 -angle * i / number_of_points + theta_stop) - thickness * np.cos(
        #                 (theta_start + theta_stop) / 2) / 2))
        return points
